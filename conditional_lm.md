...
统计模型和统计元件是不一样的，对于神经网络模型，其元件当然是统计元件（线性回归和逻辑回归），但模型却不是统计模型。典型的统计模型是使用线性模型，因为线性模型本身就是不准的，所以没法完美的解释和拟合，为了更好的解释，就牵强的说，线性模型中的系数（包括残差）是个随机变量，靠这个假设来更好的解释数据。但实际上，如果把线性模型改成合适的非线性模型，参数是可以拟合出来的确定的值，根本用不着把参数假设成随机变量。正因如此，爱因斯坦就是看不上统计模型的（虽然很多统计模型是爱因斯坦搞出来的），因为统计模型本质上是说，我不知道，所以努力凑了一个模型。

统计关心的是两个量是不是真的有差异（可以是双向的，也可以是单向的），而不关心差异的大小。这一点天生在金融上是有用的，因为金融最重要的是保证方向正确，只要方向正确，就可以通过加大杠杆来获利（长期资本公司也是这样想的，最后死于黑天鹅，，因为有黑天鹅，杠杆不是越大越好，差异也不是通过统计检验即可，差异大小还是有用的）。

20181215

静态词向量：从左向右语言模型，NNLM
静态词向量：双向语言模型，CBOW (word2vec, fasttext)
动态词向量：从左向右语言模型，ELMo, GPT
动态词向量：双向语言模型，BERT

为什么要关注别人发明的模型的细节？而且不只关注整体最优模型，还要关注其他模型的细节？
百度脑图 模型细节181215

自然语言处理的三大模块？
百度脑图 自然语言处理的分类181215
